...
async function remoteBuild (data) {
  ...
  if (statusCode !== 200) {
    ...
    const err = new Error(`${headers['x-ca-error-message']},` +
  ...
  if (!body.ok) {
    const err = new Error(`Build failed, ${body.message}`)
    err.name = 'BuildError'
    err.data = data
    throw err
  }
...
async function buildDeps (func, rootDir, type) {
  ...
  const hasPackageFile = await fs.pathExists(pkgPath)
  if (!hasPackageFile) {
  ...
  const hasZip = await fs.pathExists(zipPath)
  const hasSign = await fs.pathExists(md5Path)

  if (hasZip && hasSign) {
    const zip = await fs.readFile(zipPath, 'base64')
    const sign = await fs.readFile(md5Path, 'utf8')
  ...
  await fs.writeFile(zipPath, base64, 'base64')
  await fs.writeFile(md5Path, digest)
