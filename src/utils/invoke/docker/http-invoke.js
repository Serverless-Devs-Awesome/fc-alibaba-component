...
const ignore = require('../../ignore').isIgnored
...
class HttpInvoke extends Invoke {
  ...
  _disableRunner (evt, name) {
    let oldRunner = null
    const tmpCodeDir = this.unzippedCodeDir
    ...
    lock.acquire('invoke', (done) => {
      ...
      debug(`stop container after lock released, error: ${err}`)
  ...
  async doInvoke (req, res) {
    ...
    await lock.acquire('invoke', async () => {
      ...
      if (this.debugPort && !this.runner) {
        ...
        if (!this.isAnonymous) {
          // check signature
          if (!await validateSignature(req, res, req.method, this.credentials)) { return }
        }
